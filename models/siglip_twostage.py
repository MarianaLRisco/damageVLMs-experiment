...
class SigLIPLinearClassifier(nn.Module):
    def __init__(self, base_model, processor, classnames, temperature=0.07, description_map=None, device='cpu'):
        ...
        self.description_map = description_map if description_map is not None else {}
    ...
    def forward(self, pixel_values, no_grad_backbone=True):
        ...
        with context():
            ...
            image_embeds = vision_outputs.pooler_output #image_embeds = self.visual(pixel_values).last_hidden_state[:, 0, :]  # CLS token
    ...
    @torch.no_grad()
    def infer(self, x: Tensor, categories: list[str], template: dict | None = None, compute_classifier_once: bool = True, processor=None) -> Tensor:
        ...
        if compute_classifier_once and hasattr(self, "inference_classifier"):
            # Type ignore due to dynamic attribute access
            classifier = cast(Tensor, self.inference_classifier)  # type: ignore
        else:
            ...
            if prompts:
                ...
                with autocast_ctx:
                    text_outputs = self.text(**inputs)
                    new_embeddings = text_outputs.last_hidden_state[:, 0, :]
            ...
            # Combinar
            classifier_list = [cat2new[cat] if cat in cat2new else cat2known[cat] for cat in categories]
            classifier = torch.stack(classifier_list).to(x.device)
            classifier = F.normalize(classifier, dim=-1)


        # Embeddings visuales
        vision_outputs = self.visual(x)
...
class SigLIP2LinearClassifier(nn.Module):
    ...
    @torch.no_grad()
    def infer(self, x: Tensor, categories: list[str], template: dict | None = None, attention_mask=None, spatial_shapes=None, compute_classifier_once: bool = True, processor=None) -> Tensor:
        prompt_map = template if template is not None else self.description_map
        ...
        if compute_classifier_once and hasattr(self, "inference_classifier"):
            ...
            if prompts:
                ...
                with autocast_ctx:
                    text_outputs = self.text(**inputs)
                    new_embeddings = cast(Tensor, text_outputs.last_hidden_state[:, 0, :])
                new_embeddings = F.normalize(new_embeddings, dim=-1)
                cat2new = {cat: new for cat, new in zip(missing_classnames, new_embeddings)}
            else:
                cat2new = {}
